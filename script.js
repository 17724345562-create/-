...
function cacheEls(){
  var ids=[
    ...
    'pickerYears','pickerMonths','monthSection',
...
function resetDateTab(){
  ...
  if(el.monthSection) el.monthSection.hidden=true;
  if(el.pickerMonths) el.pickerMonths.innerHTML='';
...
function refreshDateTab(){
  ...
  if(state.year){
    buildMonths();
    if(el.monthSection) el.monthSection.hidden=false;
  }
...
function buildYears(){
  if(!el.pickerYears) return;
  el.pickerYears.innerHTML='';
  var frag=document.createDocumentFragment();
  for(var y=MIN_YEAR;y<=MAX_YEAR;y++){
    var btn=document.createElement('button');
    btn.type='button';
    btn.className='year-pick-btn'+(y===state.year?' active':'');
    btn.textContent=y;
    btn.dataset.year=y;
    btn.addEventListener('click',function(ev){
      ...
      if(el.analysisCard) el.analysisCard.hidden=true;
      try{
        if(el.monthSection) el.monthSection.scrollIntoView({behavior:'smooth',block:'nearest'});
      }catch(e){}
    });
...
function buildMonths(){
  if(!el.pickerMonths) return;
  el.pickerMonths.innerHTML='';
  var frag=document.createDocumentFragment();
  for(var i=1;i<=12;i++){
    var btn=document.createElement('button');
    btn.type='button';
    btn.className='month-pick-btn'+(i===state.month?' active':'');
    btn.textContent=i+'月';
    btn.dataset.month=i;
    btn.addEventListener('click',function(ev){
      var target=ev.currentTarget;
      ...
      try{
        if(el.daysSection) el.daysSection.scrollIntoView({behavior:'smooth',block:'nearest'});
      }catch(e){}
    });
    frag.appendChild(btn);
  }
...
function openMonth(m,keepDays){
  ...
  if(el.daysTitle) el.daysTitle.textContent=state.year+'年'+MONTH_CN[m-1];
...
function openInlineEditor(m,d){
  ...
  if(el.ieDate) el.ieDate.textContent=m+'月'+d+'日';
  fillWorkForm();
  fillLifeForm();
  refreshPayPreview();
  setSaveState('idle');
  ...
  if(el.inlineEditor) el.inlineEditor.hidden=false;
  setTimeout(function(){
...
function bindAll(){
  ...
  document.addEventListener('click',function(e){
    ...
    /* 内联编辑器 */
    if(e.target.closest('#ieClose')){ closeInlineEditor(false); return; }
  ...
  document.addEventListener('change',function(e){
    ...
    if(t.id==='rangeStart'&&rangeDraft){
      rangeDraft.type='custom';
      rangeDraft.start=t.value;
      updateRangePreview();
      return;
    }
    if(t.id==='rangeEnd'&&rangeDraft){
      rangeDraft.type='custom';
      rangeDraft.end=t.value;
      updateRangePreview();
      return;
    }
  },false);

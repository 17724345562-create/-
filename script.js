...
function renderDaysChart(m){
  ...
  var days=[];
  var maxAbs=0;
  var maxNet=-Infinity, minNet=Infinity;
  ...
  if(state.moneyHidden){
    ...
    if(hasAny&&maxDay){
      html+='<span class="cs-item high">最高 <strong>'+maxDay+'日 '+fmtNet(maxNet)+'</strong></span>';
    }
    if(hasAny&&minDay&&minDay!==maxDay&&minNet<0){
      html+='<span class="cs-item low">最低 <strong>'+minDay+'日 '+fmtNet(minNet)+'</strong></span>';
    }
    html+='</div>';
  }
  html+='</div>';
  ...
  days.forEach(function(item,i){
    var x=i*COL_W;
    var h=0;
    var isNeg=item.net<0;

    if(item.hasRec&&item.net!==0){
      h=Math.max(3, (Math.abs(item.net)/maxAbs)*(halfH-6));
    }
    ...
    var colCls='chart-col';
    if(state.day===item.d) colCls+=' selected';

    var barStyle;
    if(isNeg&&item.hasRec&&item.net!==0){
      barStyle='top:'+baselineY+'px;height:'+h+'px;';
    }else if(item.hasRec&&item.net!==0){
      barStyle='top:'+(baselineY-h)+'px;height:'+h+'px;';
    }else{
      barStyle='top:'+(baselineY-1)+'px;height:2px;';
    }
  ...
  if(viewport){
    viewport.querySelectorAll('.chart-col').forEach(function(col){
      col.addEventListener('click',function(){
        ...
        setTimeout(function(){
          ...
          if(vp) vp.scrollLeft=scrollLeftCache;
        },10);
      });
    });
    ...
    /* 记录滚动位置 */
    var scrollLeftCache=viewport.scrollLeft;
    viewport.addEventListener('scroll',function(){
      scrollLeftCache=viewport.scrollLeft;
    },{passive:true});
...
function applyAnalysisHighlight(){
  ...
  shown.forEach(function(x){
    ...
    if(cell){
      if(cell.classList.contains('chart-col')){
        cell.classList.add('highlighted',cls);
      }else{

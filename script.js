...
var KEYS={
  ...
  oldUsers:'nova_users_v2',
  oldSession:'nova_session_v2',
  oldDataPrefix:'nova_data_v2_'
};
var SECURITY={
...
function cacheEls(){
  var ids=[
    'loginScreen','loginUser','loginPass','loginBtn','loginMsg','pwdToggle','pwdStrength','pwdText','capsWarn','botSvg',
    ...
    'statMonthDays','statMonthIncome','statMonthExpense',
    'fundList','fundAddBtn','fundTotal','fundDiff','fundSaveBtn',
    'pickerTrigger','pickerValue','pickerModal','pickerClose','pickerYears','pickerMonths','pickerPreview','pickerConfirm','pickerClear',
    'daysSection','daysTitle','daysToggle','daysBody','daysGrid',
    'inlineEditor','ieDate','ieClose','editTabs','panel-work','panel-life',
    ...
    'rangeAllDesc','yearPicker','rangeStart','rangeEnd','rangeResult','rangeConfirm',
...
var state={
  ...
  moneyHidden:true,
  currentTab:'home',
  pickerYear:MIN_YEAR,pickerMonth:1,
  daysCollapsed:false
};
...
function showLoginScreen(){
  ...
  setAuthMode('login');
  setTimeout(function(){ try{ el.loginUser.focus(); }catch(e){} },120);
}
...
function refreshHomeStats(){
  ...
  el.statMonthDays.textContent=days+' 天';
  if(state.moneyHidden){
    el.statMonthIncome.textContent='¥****';
    el.statMonthExpense.textContent='¥****';
  }else{
    el.statMonthIncome.textContent='¥'+fmtMoney(income);
    el.statMonthExpense.textContent='¥'+fmtMoney(expense);
  }
}
...
function renderFunds(){
  ...
  if(!fundAccounts.length){
    ...
    fundAccounts.forEach(function(acc,i){
      ...
      item.addEventListener('click',function(){ openFundModal(i); });
...
function resetDateTab(){
  state.year=null; state.month=null; state.day=null;
  state.pickerYear=MIN_YEAR; state.pickerMonth=1;
  state.daysCollapsed=false;
  state.analysisMode=null;
  if(el.pickerValue) el.pickerValue.textContent='请选择年份和月份';
  if(el.daysSection) el.daysSection.hidden=true;
  if(el.daysTitle) el.daysTitle.textContent='请选择年月';
  if(el.daysGrid) el.daysGrid.innerHTML='';
  if(el.inlineEditor) el.inlineEditor.hidden=true;
  if(el.summaryCard) el.summaryCard.hidden=true;
  if(el.analysisCard) el.analysisCard.hidden=true;
  if(el.sumNum) el.sumNum.textContent='--';
  if(el.sumDate) el.sumDate.textContent='尚未选择日期';
  if(el.sumSub) el.sumSub.textContent='--';
  if(state.moneyHidden){
    if(el.sumIncome) el.sumIncome.textContent='¥****';
    if(el.sumExpense) el.sumExpense.textContent='¥****';
    if(el.sumBalance) el.sumBalance.textContent='¥****';
  }
  if(el.analysisTip){
    el.analysisTip.textContent='请先选择年月';
...
function updatePickerDisplay(){
  if(state.year&&state.month){
    el.pickerValue.textContent=state.year+'年'+MONTH_CN[state.month-1];
  }else{
    el.pickerValue.textContent='请选择年份和月份';
  }
}

function openPickerModal(){
  state.pickerYear=state.year||MIN_YEAR;
  state.pickerMonth=state.month||1;
  buildPickerYears();
  buildPickerMonths();
  updatePickerPreview();
  el.pickerModal.classList.add('open');
  el.pickerModal.setAttribute('aria-hidden','false');
  document.body.style.overflow='hidden';
}

function closePickerModal(){
  el.pickerModal.classList.remove('open');
  el.pickerModal.setAttribute('aria-hidden','true');
  document.body.style.overflow='';
}

function buildPickerYears(){
  el.pickerYears.innerHTML='';
  var frag=document.createDocumentFragment();
  for(var y=MIN_YEAR;y<=MAX_YEAR;y++){
    var btn=document.createElement('button');
    btn.type='button';
    btn.className='year-pick-btn'+(y===state.pickerYear?' active':'');
    btn.textContent=y;
    btn.dataset.year=y;
    btn.addEventListener('click',function(ev){
      var target=ev.currentTarget;
      state.pickerYear=Number(target.dataset.year);
      buildPickerYears();
      updatePickerPreview();
    });
    frag.appendChild(btn);
  }
  el.pickerYears.appendChild(frag);
}

function buildPickerMonths(){
  el.pickerMonths.innerHTML='';
  var frag=document.createDocumentFragment();
  for(var i=1;i<=12;i++){
    var btn=document.createElement('button');
    btn.type='button';
    btn.className='month-pick-btn'+(i===state.pickerMonth?' active':'');
    btn.textContent=i+'月';
    btn.dataset.month=i;
    btn.addEventListener('click',function(ev){
      var target=ev.currentTarget;
      state.pickerMonth=Number(target.dataset.month);
      buildPickerMonths();
      updatePickerPreview();
    });
    frag.appendChild(btn);
  }
  el.pickerMonths.appendChild(frag);
}

function updatePickerPreview(){
  el.pickerPreview.textContent=(state.pickerYear||'--')+'年'+MONTH_CN[(state.pickerMonth||1)-1];
}

function confirmPicker(){
  state.year=state.pickerYear;
  state.month=state.pickerMonth;
  state.day=1;
  loadDB(state.year);
...
function openMonth(m,keepDays){
  state.month=m;
  if(!keepDays) state.day=1;
  state.draft=null;
  state.dirty=false;
  state.analysisMode=null;
  el.daysTitle.textContent=state.year+'年'+MONTH_CN[m-1];
  buildDaysGrid(m);
  updatePickerDisplay();
  selectDay(m,state.day||1);
...
/* ============================================================
   内联编辑器
   ============================================================ */
function openInlineEditor(m,d){
  var year=state.year;
  if(!year||!m||!d) return;
  state.month=m;
  state.day=d;
  state.draft=getDay(year,m,d);
  state.dirty=false;
  state.modalOpen=true;

  el.ieDate.textContent=m+'月'+d+'日';
  fillWorkForm();
  fillLifeForm();
  refreshPayPreview();
  setSaveState('idle');
  ...
  el.inlineEditor.hidden=false;
  setTimeout(function(){
    try{ el.inlineEditor.scrollIntoView({behavior:'smooth',block:'nearest'}); }catch(e){}
  },50);
}
...
function switchTabPanel(tab){
  ...
  el.panelWork.hidden=tab!=='work';
  el.panelLife.hidden=tab!=='life';
  el.resetBtn.textContent=tab==='work'?'重置工作':'重置生活';
...
function closeWageModal(){
  ...
  if(!state.modalOpen&&!el.rangeModal.classList.contains('open')&&!el.pickerModal.classList.contains('open')&&!el.reportModal.classList.contains('open')&&!el.fundModal.classList.contains('open'))
    document.body.style.overflow='';
}
...
function buildMonthReportHTML(y,m,user){
  ...
  var rows='';
  var totalIncome=0,totalExpense=0,days=0;
  var dayList=[];
  ...
  for(var d=1;d<=n;d++){
    ...
    var income=pay+inc;
    if(dayHasRecord(day)){ days++; totalIncome+=income; totalExpense+=exp; }
    dayList.push({
      d:d,pay:pay,inc:inc,
      meal:day?calcMealTotal(day):0,
      extra:day?calcExtraTotal(day):0,
      exp:exp,income:income
    });
  }

  var avgIncome=days?totalIncome/days:0;
  var avgExpense=days?totalExpense/days:0;

  dayList.forEach(function(x){
    var wd=WD[new Date(y,m-1,x.d).getDay()];
    var incCls=x.income>avgIncome?' style="color:#e00;font-weight:700"':'';
    var expCls=x.exp>avgExpense?' style="color:#e00;font-weight:700"':'';
    rows+='<tr>'
      +'<td>'+y+'-'+String(m).padStart(2,'0')+'-'+String(x.d).padStart(2,'0')+' 周'+wd+'</td>'
      +'<td'+incCls+'>'+fmtMoney(x.pay)+'</td>'
      +'<td'+incCls+'>'+fmtMoney(x.inc)+'</td>'
      +'<td'+expCls+'>'+fmtMoney(x.meal)+'</td>'
      +'<td'+expCls+'>'+fmtMoney(x.extra)+'</td>'
  ...
  return '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40"><head><meta charset="utf-8"><title>NOVA 月度报表</title><style>body{font-family:"PingFang SC","Microsoft YaHei",sans-serif;color:#222}h1{font-size:22px;color:#0a4d8c}h2{font-size:15px;color:#555}table{border-collapse:collapse;width:100%;font-size:13px}th,td{border:1px solid #999;padding:8px;text-align:center}th{background:#e8f4ff;color:#0a4d8c}.sum{margin-top:14px;font-size:14px}.sum b{color:#0a4d8c}</style></head><body>'
    ...
    +'<p>月平均收入：<b>¥'+fmtMoney(avgIncome)+'</b></p>'
    +'<p>月平均支出：<b>¥'+fmtMoney(avgExpense)+'</b></p>'
...
function buildYearReportHTML(y,user){
  ...
  var activeMonths=monthList.filter(function(x){ return x.days>0; });
  var avgIncome=activeMonths.length?totalIncome/activeMonths.length:0;
  var avgExpense=activeMonths.length?totalExpense/activeMonths.length:0;

  monthList.forEach(function(x){
    var incCls=x.inc>avgIncome?' style="color:#e00;font-weight:700"':'';
    var expCls=x.exp>avgExpense?' style="color:#e00;font-weight:700"':'';
    rows+='<tr>'
      +'<td>'+y+'年'+x.m+'月</td>'
      +'<td>'+x.days+' 天</td>'
      +'<td'+incCls+'>'+fmtMoney(x.inc)+'</td>'
      +'<td'+expCls+'>'+fmtMoney(x.exp)+'</td>'
...
function openRangeModal(){
  ...
  el.rangeAllDesc.textContent='统计 '+MIN_YEAR+' 年至今的全部记录';
...
function closeRangeModal(){
  ...
  if(!state.modalOpen&&!el.wageModal.classList.contains('open')&&!el.pickerModal.classList.contains('open')&&!el.reportModal.classList.contains('open')&&!el.fundModal.classList.contains('open'))
    document.body.style.overflow='';
}
...
function buildRangeYearPicker(){
  ...
  for(var y=MIN_YEAR;y<=MAX_YEAR;y++){
    ...
    btn.addEventListener('click',function(ev){
      ...
      el.yearPicker.querySelectorAll('.year-pick-btn').forEach(function(x){
...
function bindAll(){
  ...
  if(el.loginBtn) el.loginBtn.addEventListener('click',doAuth);
  if(el.loginUser) el.loginUser.addEventListener('keydown',function(e){ if(e.key==='Enter') el.loginPass.focus(); });
  if(el.loginPass){
    el.loginPass.addEventListener('keydown',function(e){
      ...
      if(e.key==='Enter'){ if(state.authMode==='register') el.inviteCode.focus(); else doAuth(); }
    });
    el.loginPass.addEventListener('input',showStrength);
  }
  if(el.inviteCode) el.inviteCode.addEventListener('keydown',function(e){ if(e.key==='Enter') doAuth(); });
  if(el.pwdToggle) el.pwdToggle.addEventListener('click',function(){
  ...
  el.reportModeSeg.querySelectorAll('button').forEach(function(b){
    b.addEventListener('click',function(){ setReportMode(b.dataset.v); });
  });
  if(el.reportConfirm) el.reportConfirm.addEventListener('click',exportWord);

  /* --- 主页 --- */
  if(el.eyeBtn) el.eyeBtn.addEventListener('click',function(e){ e.stopPropagation(); toggleMoney(); });
  if(el.balanceCard) el.balanceCard.addEventListener('click',openRangeModal);

  /* --- 资金管理 --- */
  if(el.fundAddBtn) el.fundAddBtn.addEventListener('click',function(){ openFundModal(-1); });
  if(el.fundClose) el.fundClose.addEventListener('click',closeFundModal);
  if(el.fundModal) el.fundModal.addEventListener('click',function(e){ if(e.target===el.fundModal) closeFundModal(); });
  if(el.fundConfirm) el.fundConfirm.addEventListener('click',function(){
  ...
  if(el.fundDelete) el.fundDelete.addEventListener('click',function(){
  ...
  if(el.fundSaveBtn) el.fundSaveBtn.addEventListener('click',function(){
    var total=fundAccounts.reduce(function(s,a){ return s+(Number(a.amount)||0); },0);
    total=Math.round(total*100)/100;
    var balanceTotal=calcRangeTotal(state.range).balance;
    var diff=Math.round((total-balanceTotal)*100)/100;
  ...
  if(el.pickerConfirm) el.pickerConfirm.addEventListener('click',confirmPicker);
  if(el.pickerClear) el.pickerClear.addEventListener('click',clearPickerSelection);

  /* --- 日期收起 / 展开 --- */
  if(el.daysToggle) el.daysToggle.addEventListener('click',function(){
  ...
  /* --- 内联编辑器 --- */
  if(el.ieClose) el.ieClose.addEventListener('click',function(){ closeInlineEditor(false); });
  el.editTabs.querySelectorAll('.tab').forEach(function(t){
    t.addEventListener('click',function(){ switchTabPanel(t.dataset.tab); });
  });
  if(el.resetBtn) el.resetBtn.addEventListener('click',resetCurrentTab);
  if(el.locInput) el.locInput.addEventListener('input',function(e){
    state.draft.work.location=e.target.value;
  ...
  if(el.startTime) el.startTime.addEventListener('change',function(e){
    state.draft.work.start=e.target.value;
    enableWork(); markDirty(); updateHoursBadge(); refreshPayPreview();
  });
  if(el.endTime) el.endTime.addEventListener('change',function(e){
    state.draft.work.end=e.target.value;
    enableWork(); markDirty(); updateHoursBadge(); refreshPayPreview();
  });
  if(el.addBreak) el.addBreak.addEventListener('click',function(){
  ...
  document.querySelectorAll('.meal-input').forEach(function(inp){
    inp.addEventListener('input',function(e){
      var key=e.target.dataset.meal;
      state.draft.life.meals[key].amount=e.target.value;
      enableLife(); markDirty(); updateMealBadge(); updateLifeBalance();
    });
  });
  if(el.addExtra) el.addExtra.addEventListener('click',function(){
    if(!state.draft.life.extras) state.draft.life.extras=[];
    var idx=state.draft.life.extras.length;
    var ne={name:'',amount:'',photo:''};
    state.draft.life.extras.push(ne);
    if(idx===0) el.extraList.innerHTML='';
    appendExtraItem(ne,idx);
    enableLife(); markDirty(); updateExtraBadge(); updateLifeBalance();
    var inputs=el.extraList.querySelectorAll('.extra-name');
    if(inputs.length) inputs[inputs.length-1].focus();
  });
  if(el.addIncome) el.addIncome.addEventListener('click',function(){
    if(!state.draft.life.incomes) state.draft.life.incomes=[];
    var idx=state.draft.life.incomes.length;
    var ne={name:'',amount:'',photo:''};
    state.draft.life.incomes.push(ne);
    if(idx===0) el.incomeList.innerHTML='';
    appendIncomeItem(ne,idx);
    enableLife(); markDirty(); updateIncomeBadge(); updateLifeBalance();
    var inputs=el.incomeList.querySelectorAll('.extra-name');
    if(inputs.length) inputs[inputs.length-1].focus();
  });

  /* --- 分析 --- */
  if(el.analysisReset) el.analysisReset.addEventListener('click',function(){
    state.analysisMode=null;
  ...
  document.querySelectorAll('.analysis-item').forEach(function(btn){
    btn.addEventListener('click',function(){
      ...
      if(state.analysisMode===mode){
        ...
        document.querySelectorAll('.analysis-item').forEach(function(x){ x.classList.toggle('active',x===btn); });
        el.analysisTip.textContent='已高亮：'+btn.querySelector('.ai-label').textContent;
        el.analysisTip.classList.add('active');
      }
      applyAnalysisHighlight();
    });
  ...
  /* --- 工资模板 --- */
  if(el.payBlock) el.payBlock.addEventListener('click',openWageModal);
  if(el.wageClose) el.wageClose.addEventListener('click',closeWageModal);
  if(el.wageModal) el.wageModal.addEventListener('click',function(e){ if(e.target===el.wageModal) closeWageModal(); });
  el.wageModeSeg.querySelectorAll('button').forEach(function(b){
    b.addEventListener('click',function(){
      wageDraft.mode=b.dataset.v;
      el.wageModeSeg.querySelectorAll('button').forEach(function(x){ x.classList.toggle('active',x===b); });
      el.fieldHourly.hidden=wageDraft.mode!=='hourly';
      el.fieldBase.hidden=wageDraft.mode!=='base';
      updateWagePreview();
    });
  });
  [el.hourlyRate,el.baseSalary,el.monthDays,el.stdHours].forEach(function(inp){
    if(!inp) return;
  ...
  if(el.addAllow) el.addAllow.addEventListener('click',function(){
    wageDraft.allowances.push({name:'',amount:''});
    renderAllowances();
  });
  if(el.wageConfirm) el.wageConfirm.addEventListener('click',function(){
  ...
  /* --- 保存 --- */
  if(el.saveBtn) el.saveBtn.addEventListener('click',function(){
    var k=keyOf(state.year,state.month,state.day);
    ...
    if(ok){
      ...
      refreshHome();
      setTimeout(function(){ closeInlineEditor(true); },300);
    }else{
  ...
  document.addEventListener('click',function(e){
    ...
    var box=e.target.closest('.photo-box');
    if(!box) return;
    e.preventDefault();
    var kind=box.dataset.kind;
    if(!kind) return;
    if(kind==='meal') pickPhoto({kind:'meal',key:box.dataset.key});
    else if(kind==='extra') pickPhoto({kind:'extra',index:Number(box.dataset.index)});
    else if(kind==='income') pickPhoto({kind:'income',index:Number(box.dataset.index)});
  },false);
  ...
  /* --- 范围弹窗 --- */
  if(el.rangeClose) el.rangeClose.addEventListener('click',closeRangeModal);
  if(el.rangeModal) el.rangeModal.addEventListener('click',function(e){ if(e.target===el.rangeModal) closeRangeModal(); });
  el.rangeTabs.forEach(function(t){
    t.addEventListener('click',function(){
      var type=t.dataset.range;
      el.rangeTabs.forEach(function(x){ x.classList.toggle('active',x===t); });
  ...
  if(el.rangeStart) el.rangeStart.addEventListener('change',function(){
    if(el.rangeStart.value){ rangeDraft.type='custom'; rangeDraft.start=el.rangeStart.value; updateRangePreview(); }
  });
  if(el.rangeEnd) el.rangeEnd.addEventListener('change',function(){
    if(el.rangeEnd.value){ rangeDraft.type='custom'; rangeDraft.end=el.rangeEnd.value; updateRangePreview(); }
  });
  if(el.rangeConfirm) el.rangeConfirm.addEventListener('click',function(){
    if(rangeDraft.type==='custom'){
      var s=el.rangeStart.value||(MIN_YEAR+'-01');
      var e=el.rangeEnd.value||((state.year||MIN_YEAR)+'-12');
      if(s>e){ showToast('开始月份不能晚于结束月份',true); return; }
      state.range={type:'custom',start:s,end:e};
  ...
  document.addEventListener('keydown',function(e){
    ...
    else if(el.pickerModal&&el.pickerModal.classList.contains('open')) closePickerModal();
    else if(state.modalOpen) closeInlineEditor(true);
  });

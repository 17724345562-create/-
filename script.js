...
function renderDaysList(m){
  ...
  for(var d=1;d<=n;d++){
    ...
    var metaText;
    if(hasRec){
      metaText=state.year+'年'+m+'月'+d+'日 · 周'+wdCN;
    }else{
      metaText=ev ? (state.year+'年'+m+'月'+d+'日 · '+ev) : (state.year+'年'+m+'月'+d+'日 · 未记录');
    }

    var incText=state.moneyHidden?'•••':('收 ¥'+fmtMoney(income));
    var expText=state.moneyHidden?'•••':('支 ¥'+fmtMoney(expense));
    ...
    html+='<div class="'+rowCls+'" data-day="'+d+'">'
      ...
      +'</div>'
      +'<div class="dr-main">'
        +'<div class="dr-meta">'+escapeHtml(metaText)+'</div>'
        +'<div class="dr-values">'
          +'<span class="'+incCls+'">'+incText+'</span>'
          +'<span class="'+expCls+'">'+expText+'</span>'
        +'</div>'
      +'</div>'

...
function renderFunds(){
  ...
  /* 堆叠条：0 元 或 隐藏金额时隐藏 */
  if(el.fundStack){
    if(totalAll<=0 || state.moneyHidden){
      el.fundStack.hidden=true;
  ...
  if(!fundAccounts.length){
    ...
    var sorted=fundAccounts.map(function(a,i){ return {acc:a, idx:i, amt:Number(a.amount)||0}; })
      .sort(function(a,b){ return b.amt-a.amt; });

    sorted.forEach(function(item){
      var acc=item.acc, i=item.idx;
      var btn=document.createElement('button');
      btn.type='button';
      btn.className='fund-item';
      btn.dataset.fundIndex=String(i);

      var dotColor=FUND_COLORS[i%FUND_COLORS.length];
      var amtText=state.moneyHidden?'¥****':('¥'+fmtMoney(acc.amount||0));

      btn.innerHTML='<span class="fund-item-left">'
        +'<span class="fund-color-dot" style="background:'+dotColor+';color:'+dotColor+'"></span>'
  ...
  if(el.fundTotal){
    el.fundTotal.textContent=state.moneyHidden?'¥****':('¥'+fmtMoney(totalAll));
  }

  if(el.fundDiff){
    if(fundDirty){
      el.fundDiff.textContent='● 未保存';
      el.fundDiff.className='fs-hint dirty';
    }else{
      el.fundDiff.textContent='✓ 已保存';
      el.fundDiff.className='fs-hint saved';
    }
  }
...
function renderDaysList(m){
  ...
  for(var d=1;d<=n;d++){
    ...
    var ev=(MONTH_EVENTS[m]||{})[d];
    var hasData=(state.year===DBYear?DB[k]:readYearData(state.year)[k]);
    var income=0,expense=0,net=0,hasRec=false;
    if(hasData){
      var day=getDay(state.year,m,d);
      hasRec=dayHasRecord(day);
      if(hasRec){
        income=Math.round((calcPay(day)+calcIncomeTotal(day))*100)/100;
        expense=Math.round(calcExpense(day)*100)/100;
        net=Math.round((income-expense)*100)/100;
      }
    }

    var date=new Date(state.year,m-1,d);
    var wdCN=['日','一','二','三','四','五','六'][date.getDay()];

    var rowCls='day-row'+(hasRec?'':' empty');
    if(state.day===d) rowCls+=' active';

    var netCls='dr-net';
    if(!hasRec) netCls+=' empty';
    else if(net<0) netCls+=' negative';
    else if(net===0) netCls+=' zero';

    var netText=hasRec
      ? (state.moneyHidden ? '•••' : (net>=0?'+':'-')+'¥'+fmtMoney(Math.abs(net)))
      : '—';

    var incCls='dr-income'+(income===0?' zero':'');
    var expCls='dr-expense'+(expense===0?' zero':'');

    var dateText=state.year+'年'+m+'月'+d+'日 · 周'+wdCN;
    var subText=hasRec ? '' : (ev || '');

    var incText=state.moneyHidden?'•••':('¥'+fmtMoney(income));
    var expText=state.moneyHidden?'•••':('¥'+fmtMoney(expense));

    html+='<div class="'+rowCls+'" data-day="'+d+'">'
      +'<div class="dr-day-block">'
        +'<span class="dr-day-num">'+(d<10?'0'+d:d)+'</span>'
        +'<span class="dr-day-wd">周'+wdCN+'</span>'
      +'</div>'
      +'<div class="dr-main">'
        +'<div class="dr-date">'+escapeHtml(dateText)
          +(subText?'<span class="dr-tag">'+escapeHtml(subText)+'</span>':'')
        +'</div>'
        +'<div class="dr-values">'
          +'<span class="dr-inc">收 <em class="'+incCls+'">'+incText+'</em></span>'
          +'<span class="dr-exp">支 <em class="'+expCls+'">'+expText+'</em></span>'
        +'</div>'
      +'</div>'
